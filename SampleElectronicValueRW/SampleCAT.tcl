...
# Simulator for an EFT device, for handling with an ElectronicValueRW service. Communication happens
...
# This script is a copy of the EFT simulator for handling with a CAT service. It demonstrates that handling such
# devices via ElectronicValueRW might be a better solution for ETF services due to less vendor-specific solutions.
# However, since this is a copy of the EFT simulator for CAT services, you can use that simulator as well.
	...
	if {$Status > 2} {
		...
		if {[string length $cardno] == 16 && [string length $expdate] == 4} {
			...
			if {$issuer != ""} {
				...
				set yeardelta [expr ($actyear - $year) % 100]
				if {(($yeardelta > 0 && $yeardelta < 10) || ($yeardelta == 0 && $month >= $actmonth)) && $month >= 1 && $month <= 12 } {
					set CardState 1
