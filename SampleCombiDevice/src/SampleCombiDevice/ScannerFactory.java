...
/**
 * Factory class for combined device sample scanner implementation
 */
public class ScannerFactory extends Factory implements JposServiceInstanceFactory {
    @Override
    public JposServiceInstance createInstance(String s, JposEntry jposEntry) throws JposException {
        try {
            ...
            if (deviceClass.equals("Scanner")) {
                ...
                dev.checkRange(index, 0, dev.Scanners.length - 1, JposConst.JPOS_E_ILLEGAL, "Scanner index out of range");
                dev.checkProperties(jposEntry);
                JposServiceInstance scanner = addDevice(index, dev);
                if (create) {
                    putDevice(port, dev);
                }
                return scanner;
