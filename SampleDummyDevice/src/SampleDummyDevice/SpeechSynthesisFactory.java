...
/**
 * Factory class for sample SpeechSynthesis device implementation
 */
@SuppressWarnings("unused")
public class SpeechSynthesisFactory extends Factory implements JposServiceInstanceFactory {
    @Override
        ...
        try {
            ...
            synchronized(Devices) {
                if (deviceClass.equals("SpeechSynthesis")) {
                    JposDevice any = getDevice("Any");
                    SpeechSynthesis dev;
                    boolean create = any == null;
                    if (create) {
                        dev = new SpeechSynthesis("Any");
                    } else if (!(any instanceof SpeechSynthesis))
                        throw new JposException(JPOS_E_NOSERVICE, "Different devices on same port: Any");
                    else {
                        dev = (SpeechSynthesis) any;
                    }
